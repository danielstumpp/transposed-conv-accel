...
#pragma HLS interface s_axilite port = return bundle = control
    ...
    //#pragma HLS array_partition variable=out_block dim=3 factor=2 cyclic
                ...
                for (int h = 0; h < CFG::osTile; ++h){
                    for (int w = 0; w < CFG::osTile; ++w){
                        for (int ii = 0; ii < CFG::ocTile/WIDTH512; ++ii){
                            ...
                            out[(h + ht) * CFG::out_size * CFG::out_channels/WIDTH512 + (w + wt) * CFG::out_channels/WIDTH512 + (ii + it/WIDTH512)] = out_temp;
                        }
                    }
                }
            }  
